   ...
   Input: BED/GFF3 file, file format, feature type
    ...
    if format=='bed':
        ...
        if featuretype is not None:
            gen_range=gen_range[gen_range.Feature.isin(featuretype)] #Produces rows with only the desired feature type (default: region)
        
        gen_range=gen_range.drop(['Source','Feature','Score','Frame']) #Drop unnecessary columns
        
        return gen_range #If file is in .gff3 format
    
    else: #Raise value error if unsupported format entered
        raise ValueError(f'Invalid interval format {format}. Please re-enter either gff3 or bed and try again!')
