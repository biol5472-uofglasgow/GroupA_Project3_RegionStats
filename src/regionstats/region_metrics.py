...
def n_fraction(seq: str) -> float:
    ...
    seq_upper: str = seq.upper() #standardisation of the sequence by converting every base into a upper case 
...
def gc_fraction(seq: str, mode: str = "include_n") -> float:
    ...
    total_len: int = len(seq_upper) #Calculates the length of a given region including the Ns
    gc_fraction:float=0.0
    #If the mode given is exclude_n then the operations are as follows 
    
    if mode == "exclude_n": 
        
        effective_length: int = total_len - n_count #Substracts the number of Ns from the total length 
        if effective_length == 0: #If the entire stretch of the sequence comprises of Ns 
